...
Basic syntax:
```bash
python pixelart.py <input_image> <output_image> [options]
```

### Examples

**1. Basic Conversion**
Pixelates the image into 8x8 blocks and saves it as a high-resolution, blocky image.
```bash
python pixelart.py photo.jpg result.png
```

**2. Custom Palettes (New!)**
Map your image to specific hex codes. Great for recreating retro console aesthetics.
* *Classic Game Boy:*
  ```bash
  python pixelart.py photo.jpg result.png --size 8 --palette "#0f380f,#306230,#8bac0f,#9bbc0f"
...
**3. Change Pixel Size & Limit Adaptive Colors**
...
**5. Export Actual Tiny Resolution ("Small Download")**
...
| `output`     | String  | Required| Path to save the generated pixel art image. |
